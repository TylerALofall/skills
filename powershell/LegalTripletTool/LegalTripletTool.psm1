...
function Resolve-LegalSeedLinks {
    ...
    foreach ($item in $Collection) {
        if ($item.ContentType -eq 'xml') {
            $factNodes = Get-NestedValue -Object $item.Content -Path $XmlFactPath
            if ($null -ne $factNodes) {
                $factList = if ($factNodes -is [System.Collections.IEnumerable] -and $factNodes -isnot [string]) { @($factNodes) } else { @($factNodes) }
                foreach ($entry in $factList) {
                    $facts.Add([pscustomobject]@{
                        FactId = Get-NestedValue -Object $entry -Path $XmlFactIdProperty
                        File = $item.Path
                        Statement = (Get-NestedValue -Object $entry -Path 'Statement')
                        SourceKind = 'XMLFact'
                    })
                }
            }
            ...
            if ($data -is [System.Collections.IEnumerable] -and $data -isnot [string]) {
                foreach ($entry in $data) {
                    $responses.Add([pscustomobject]@{
                        ArgId = Get-NestedValue -Object $entry -Path $JsonArgIdProperty
                        SeedArgId = Get-NestedValue -Object $entry -Path $JsonSeedArgIdProperty
                        TempFactId = (Get-NestedValue -Object $entry -Path 'temp_fact_id')
                        File = $item.Path
                        DescriptionShort = (Get-NestedValue -Object $entry -Path 'description_short')
...
function Build-EvidenceSourceXml {
    ...
    param(
        ...
        [switch]$NormalizeFactIds,
        [switch]$NormalizeDates,
        [switch]$AddActionFromEvent
    )
    ...
    foreach ($fact in $facts) {
        ...
        if ($NormalizeFactIds -and -not [string]::IsNullOrWhiteSpace($rawId) -and -not $rawId.StartsWith("$docket")) {
            $fact.id = "$docket`_$rawId"
        }

        if ($NormalizeDates -and $fact.Temporal -and $fact.Temporal.Date) {
            $d = ConvertTo-NormalizedDate -Value ([string]$fact.Temporal.Date)
            if ($d) { $fact.Temporal.Date = $d }
        }
        if ($NormalizeDates -and $fact.Temporal -and ($fact.Temporal.PSObject.Properties.Name -contains 'Date2') -and $fact.Temporal.Date2) {
            $d2 = ConvertTo-NormalizedDate -Value ([string]$fact.Temporal.Date2)
            if ($d2) { $fact.Temporal.Date2 = $d2 }
        }

        if ($AddActionFromEvent -and $fact.Triplet) {
            if (($fact.Triplet.PSObject.Properties.Name -contains 'Event') -and $fact.Triplet.Event -and -not ($fact.Triplet.PSObject.Properties.Name -contains 'Action')) {
    ...
    $inputNode = $doc.CreateElement('InputFile')
    $inputNode.InnerText = (Resolve-Path $SourceXmlPath).Path
    $pipeline.AppendChild($inputNode) | Out-Null
    ...
    foreach ($target in $targetsUnique) {
        ...
        if ($EvidenceFilesRoot -and (Test-Path $EvidenceFilesRoot)) {
            $needle = $target -replace '[^A-Za-z0-9-]',''
             $targetMatches = @(Get-ChildItem -Path $EvidenceFilesRoot -File -Recurse | Where-Object { $_.BaseName -match [regex]::Escape($needle) -or $_.Name -match [regex]::Escape($needle) })
            $present = $targetMatches.Count -gt 0
        }
        ...
        $checkRows.Add([pscustomobject]@{
            ...
            Matches = @($targetMatches | Select-Object -ExpandProperty FullName)
        })
    }
    ...
    $checkpointObj = [pscustomobject]@{
        ...
        TransformFlags = @{ NormalizeFactIds = [bool]$NormalizeFactIds; NormalizeDates = [bool]$NormalizeDates; AddActionFromEvent = [bool]$AddActionFromEvent }
    }
